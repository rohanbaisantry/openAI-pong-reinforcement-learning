...
# Training an AI to play the atari game pong using Reinforcement Learning and Neural Networks.
...
# upgrade using gradient and RMSProp
...
# Use the error to calculate the gradient
    # The below dimensions all assume we had exactly "batch_size" frames in the round (not necessarily true!)
    # dC_dw2 = hidden_layer_values^T dot gradient_log_p ([1 x 2000] dot [2000 x 1] -> 1x1)
    # delta_1 = gytyradient_log_p outer_product weights['2'] = [2000 x 1] outer_product [1 x 200] ([2000 x 200])
...
def apply_neural_nets(observation_matrix, weights):
    ...
    output_layer_values = sigmoid(output_layer_values) # output_layer_values = softmax(output_layer_values)
    return hidden_layer_values, output_layer_values

def choose_action(probability):
    """ m = np.max(probability)
    for i in range(6):
        if probability[i] == m:
            return i """
...
def compute_gradient(gradient_log_p, hidden_layer_values, observation_values, weights):
    ...
    dC_dw2 = np.dot(hidden_layer_values.T, delta_L).ravel() #flatten
...
def main():
    ...
    weights = {
        ...
        '2': np.random.randn(num_hidden_layer_neurons) / np.sqrt(num_hidden_layer_neurons) # '2': np.random.randn(6, num_hidden_layer_neurons) / np.sqrt(num_hidden_layer_neurons)  
    }
    ...
    while True:
        frames.append(env.render()) # to help with displaying.
        print(observation) # so we can see the state in the prompt
        processed_observations, prev_processed_observations = preprocess_observations(observation, prev_processed_observations, input_dimensions)
        hidden_layer_values, action_probability = apply_neural_nets(processed_observations, weights)
